...
class BookingSpider(scrapy.Spider):
    ...
    custom_settings = {
        ...
        # Disable selenium middleware for now
        ...
        for hotel in hotel_elements:
            try:
                ...
                if hotel_data['name']:
                    ...
                    cleaned_data = self.clean_hotel_data(hotel_data)
                    
                    # Populate item fields
                    item['name'] = cleaned_data.get('name')
                    item['location'] = cleaned_data.get('location')
                    item['url'] = cleaned_data.get('url')
                    item['image_url'] = cleaned_data.get('image_url')
                    item['price'] = cleaned_data.get('price')
                    item['price_numeric'] = cleaned_data.get('price_numeric')
                    item['rating'] = cleaned_data.get('rating')
                    item['rating_numeric'] = cleaned_data.get('rating_numeric')
                    item['reviews_count'] = cleaned_data.get('reviews_count')
                    item['reviews_count_numeric'] = cleaned_data.get('reviews_count_numeric')
                    item['source'] = 'html'
                    item['scraped_at'] = datetime.now().isoformat()
                    item['search_destination'] = self.destination
                    item['search_checkin'] = self.checkin
                    item['search_checkout'] = self.checkout
    ...
    def parse_graphql_hotel(self, result):
        ...
        try:
            ...
            # Create BookingHotelItem
            item = BookingHotelItem()
            item['id'] = basic_data.get('id')
            item['name'] = basic_data.get('name')
            item['location'] = basic_data.get('location', {}).get('displayLocation')
            item['rating_numeric'] = basic_data.get('reviewScore', {}).get('score')
            item['reviews_count_numeric'] = basic_data.get('reviewScore', {}).get('totalScoreCount')
            item['price_numeric'] = price_amount
            item['currency'] = price_currency
            item['page_name'] = basic_data.get('pageName')
            item['source'] = 'graphql'
            item['scraped_at'] = datetime.now().isoformat()
            item['search_destination'] = self.destination
            item['search_checkin'] = self.checkin
            ...
            if photos and photos.get('main', {}).get('highResUrl'):
                item['image_url'] = photos['main']['highResUrl'].get('absoluteUrl')
            
            # Create booking URL
            if item['page_name']:
                item['url'] = f"https://www.booking.com/hotel/{item['page_name']}.html"
